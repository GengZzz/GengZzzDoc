...
const examples: Record<
  SelectorType,
  { title: string; items: { css: string; match: string; desc: string }[] }
> = {
  basic: {
    title: '基础选择器',
    items: [
      ...
      { css: '.card', match: 'class="card" 的元素', desc: '类选择器，优先级 (0,1,0)，最常用' },
      {
        css: '#header',
        match: 'id="header" 的元素',
        desc: 'ID 选择器，优先级 (1,0,0)，一个页面中应唯一',
      },
      {
        css: '[type="text"]',
        match: 'type="text" 的元素',
        desc: '属性选择器，优先级 (0,1,0)，常用于表单',
      },
  ...
  'pseudo-class': {
    ...
    items: [
      ...
      { css: 'input:focus', match: '获得焦点的输入框', desc: ':focus 获得焦点，用于表单样式' },
      {
        css: 'li:first-child',
        match: '第一个 li 元素',
        desc: ':first-child 父元素下的第一个子元素',
      },
      {
        css: 'li:nth-child(2n)',
        match: '第 2、4、6... 个 li',
        desc: ':nth-child() 支持公式，实现斑马纹',
      },
  ...
  'pseudo-element': {
    ...
    items: [
      { css: 'p::first-line', match: '段落的第一行', desc: '::first-line 选中第一行文本' },
      {
        css: 'p::first-letter',
        match: '段落的首字母',
        desc: '::first-letter 选中首字母，杂志风格',
      },
      {
        css: '.tip::before',
        match: '在 .tip 内容前插入',
        desc: '::before 在元素内部最前面插入内容',
      },
...
<template>
  <div class="selector-demo">
    ...
    <div class="table">
      ...
      <div v-for="(item, i) in examples[selected].items" :key="i" class="row">
        <span class="col-css"
          ><code>{{ item.css }}</code></span
        >
