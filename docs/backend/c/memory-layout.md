---
title: "内存布局"
description: "理解程序的地址空间布局是理解指针、栈溢出、堆溢出和段错误的基础。每个 C 程序运行时，操作系统为其映射一块虚拟地址空间。"
---
...
<CStackFrameOverflowDemo />

动画里的布局是简化模型，真实栈帧会受编译器、优化级别、ABI、保护选项影响，但核心风险不变：越界写入不会停在数组边界，它会继续覆盖相邻栈数据。轻则变量被改坏，重则返回地址被破坏，程序崩溃或形成漏洞。
