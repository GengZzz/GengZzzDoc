...
> **最高优先级**：`content-standard.md` 顶部的「写作设计三原则」（深意 / 结构 / 易懂）统领一切。任何细则与之冲突时以三原则为准，写作前先读它。
...
4. **动笔前先定主线**：用一句话写出本篇要回答的**核心问题**，并列出小标题草稿，确认它们构成一条逻辑链（叙事弧：痛点 → 朴素做法 → 正解 → 本质 → 边界）。主线不清不要开始写。
5. 写完后更新该 index 的清单状态与「学习路径」。

## 二、单篇健壮性 Checklist

每篇先过 `content-standard.md` 的「写作设计三原则」与三段式（概念+为什么 → 渐进示例 → 取舍/误区），再逐项满足：

- [ ] **三原则自检**：深意（能讲清「为什么这样设计」）、结构（小标题串起来是条逻辑链）、易懂（新手能顺下来）
- [ ] **主线清晰**：开篇点明核心问题，全篇围绕它，无前向引用
- [ ] **frontmatter**：`title` + 一句话 `description`（SEO/搜索摘要）
- [ ] **开篇即场景**：第一段先回答「解决什么问题、没有它会怎样」
- [ ] **渐进示例**：最小可运行 → 接近真实，两级递进
- [ ] **取舍与边界**：讲清代价、适用边界、什么时候不该用
- [ ] **类比/心智模型**：陌生概念给类比，并点明类比边界
- [ ] **纠错心智模型**：显式写出常见误解「你可能以为…，其实…」
...
- **三原则自检**全部为「是」（深意 / 结构 / 易懂，见 `content-standard.md` 5.0）
- 板块 index 的完成度清单已更新
- 单篇通过第二节 Checklist
